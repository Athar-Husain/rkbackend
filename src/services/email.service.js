...
export const sendEmail_production = async ({
  to,
  subject,
  html,
  text,
...
export const sendEmail = async ({
  ...
  try {
    // 1. Check for Development/Mock mode
    // You can set MOCK_EMAIL=true in your .env file
    if (
      ...
      // Extracting OTP from text or html if it exists for easy debugging
      const otpMatch = (html || text || "").match(/\b\d{6}\b/);
      if (otpMatch) console.log(`[MOCK OTP]: ${otpMatch[0]}`);
      console.log("-----------------------------------------");

      // Return identical structure to the real service
      return {
        success: true,
    ...
    // 2. Real SMTP Logic (only runs in production)
    if (!to || !subject || (!html && !text)) {
      throw new Error("Missing required email parameters");
    }
