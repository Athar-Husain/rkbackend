...
exports.sendOTP = async (req, res, next) => {
  try {
    ...
    const isResend = await OTPService.checkOTPExists(mobile);

    // Send OTP
    const result = await OTPService.sendOTP(mobile, isResend);
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    const result = await OTPService.generatePurposeOTP(
      mobile,
      "PASSWORD_RESET",
      30
    );
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    // Verify OTP
    const otpResult = await OTPService.verifyPurposeOTP(
      mobile,
      "PASSWORD_RESET",
      otp
    );
