...
export const getHomeDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      Promotion.getFeaturedPromotions(5),

      userId
        ? Promotion.getPromotionsForUser(userId, { limit: 10 })
        : Promotion.getActivePromotions({ limit: 10 }),
      ...
      Product.find({
        ...
        .limit(10)
        .select(
          "name slug mrp sellingPrice discountPercentage images category brand",
        )
        .lean(),

      userId
        ? User.findById(userId)
            .select("name email mobile walletBalance referralCode")
            .lean()
        : null,
    ]);
...
export const getPromotions2 = async (req, res) => {
  try {
