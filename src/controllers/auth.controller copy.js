...
export const signupVerifyOTP = async (req, res) => {
  try {
    ...
    await verifyOTP({
      identifier: tempUser.email, // Use the email from tempUser (should match exactly)
      type: "email",
      otp,
      purpose: "SIGNUP",
    });
...
export const signinSendOTPNew = async (req, res) => {
  try {
    console.log("htiiiit");
    ...
    if (user.isBlocked) return fail(res, 403, "Account bloacked contact Admin");
    ...
    console.error("Signin send OTP error:", error.message);
    return fail(res, 400, error.message);
  }
};
...
export const signinSendOTP2 = async (req, res) => {
  try {
    const { emailOrMobile } = req.body;
    ...
    // Make sure email is included in the token
    const tempToken = jwt.sign(
      {
        userId: user._id,
        email: user.email, // ✅ Make sure email is included
        mobile: user.mobile, // ✅ Include mobile too
        purpose: "LOGIN",
      },
      process.env.JWT_TEMP_SECRET,
      { expiresIn: "10m" },
...
export const signinVerifyOTP1 = async (req, res) => {
  try {
    const {
      otp,
      tempToken,
    ...
    // Register FCM token
    if (deviceToken) {
      await registerOrUpdateFCMToken(user._id, deviceToken, platform, deviceId);
    }
    ...
    logger.error("Signin verify OTP error:", error);
...
export const signinVerifyOTP2 = async (req, res) => {
  try {
    console.log("req body in signinVerifyOTP", req.body);
